...
KHULNASOFT_AGENT_PIPELINE_URL = os.environ['KHULNASOFT_AGENT_PIPELINE_URL'].rstrip('/')
...
def _get_jobs(pipeline_id, scope=None):
    all_jobs = []
    url = f"{KHULNASOFT_AGENT_PIPELINE_URL}/pipelines/{pipeline_id}/jobs"
    while True:
...
def retry_failed_jobs(pipeline_id):
    ...
    for job in failed_jobs:
        url = f"{KHULNASOFT_AGENT_PIPELINE_URL}/jobs/{job['id']}/retry"
