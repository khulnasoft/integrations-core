...
from khulnasoft_checks.airflow import AirflowCheck
...
def test_service_checks_healthy_exp(aggregator, json_resp, expected_healthy_status, expected_healthy_value):
    ...
    with mock.patch('khulnasoft_checks.airflow.airflow.AirflowCheck._get_version', return_value=None):

        with mock.patch('khulnasoft_checks.base.utils.http.requests') as req:
...
def test_service_checks_healthy_stable(
    ...
    with mock.patch('khulnasoft_checks.airflow.airflow.AirflowCheck._get_version', return_value='2.6.2'):
