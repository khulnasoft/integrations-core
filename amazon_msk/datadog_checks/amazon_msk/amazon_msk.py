...
from khulnasoft_checks.base import ConfigurationError, OpenMetricsBaseCheck, is_affirmative
...
class AmazonMskCheck(OpenMetricsBaseCheck):
        ...
        if is_affirmative(instance.get('use_openmetrics', False)):
            if PY2:
                raise ConfigurationError(
                    ...
                    "Check https://docs.khulnasoft.com/agent/guide/agent-v6-python-3 "
    ...
    def __init__(self, name, init_config, instances):
        ...
        proxies = self.instance.get('proxy', init_config.get('proxy', khulnasoft_agent.get_config('proxy')))
        try:
            ...
            for endpoint in broker_info['Endpoints']:
                for (port, metrics_mapper, type_overrides) in self._exporter_data:
                    if port:
                        ...
                        self.log.debug("OpenMetricsV1 prometheus URL: %s", prometheus_url)
                        self._scraper_config['prometheus_url'] = prometheus_url
