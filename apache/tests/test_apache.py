...
from khulnasoft_checks.apache import Apache
...
@pytest.mark.usefixtures("dd_environment")
def test_metadata_in_content(check, khulnasoft_agent):
    check = check(AUTO_CONFIG)
...
@pytest.mark.usefixtures("dd_environment")
def test_metadata_in_header(check, khulnasoft_agent, mock_hide_server_version):
...
def test_full_version_regex(check, version, expected_parts, khulnasoft_agent):
...
def test_scoreboard_values(aggregator, check, scoreboard, expected_metrics, khulnasoft_agent):
    """The default server token is Full. Full results in server info that can include
    multiple non-Apache version specific information.
    """
    check = check({})
