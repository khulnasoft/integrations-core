...
from khulnasoft_checks.arangodb import ArangodbCheck
...
def test_get_server_tag(instance, caplog, side_effect, log_message):
    caplog.clear()
    check = ArangodbCheck('arangodb', {}, [instance])
    with mock.patch("khulnasoft_checks.base.utils.http.RequestsWrapper.get", side_effect=side_effect):
...
def test_refresh_scrapers(instance, server_tags, args):
    check = ArangodbCheck('arangodb', {}, [instance])
    with mock.patch("khulnasoft_checks.arangodb.check.ArangodbCheck.get_server_tag") as mock_get_server_tag:
