...
from khulnasoft_checks.clickhouse import ClickhouseCheck, queries
...
@mock.patch('khulnasoft_checks.base.AgentCheck.is_metadata_collection_enabled', return_value=False)
def test_can_connect_submits_on_every_check_run(is_metadata_collection_enabled, aggregator, instance):
    ...
    """
    check = ClickhouseCheck('clickhouse', {}, [instance])
    with mock.patch("khulnasoft_checks.clickhouse.clickhouse.clickhouse_driver"):
...
def test_can_connect_recovers_after_failed_connection(is_metadata_collection_enabled, aggregator, instance):
    ...
    with mock.patch('clickhouse_driver.Client', side_effect=NetworkError('Connection refused')):
        with mock.patch('khulnasoft_checks.clickhouse.ClickhouseCheck.ping_clickhouse', return_value=False):
            with pytest.raises(Exception):
                check.check({})
            with pytest.raises(Exception):
                check.check({})
...
def test_can_connect_recovers_after_failed_ping(is_metadata_collection_enabled, aggregator, instance):
    ...
    with mock.patch("khulnasoft_checks.clickhouse.clickhouse.clickhouse_driver"):
        check.check({})
        with mock.patch('khulnasoft_checks.clickhouse.ClickhouseCheck.ping_clickhouse', side_effect=Error()):
