...
from khulnasoft_checks.base.types import ServiceCheck
from khulnasoft_checks.cloudera.metrics import TIMESERIES_METRICS
...
def test_list_hosts(
    ...
    with mock.patch(
        'khulnasoft_checks.cloudera.client.cm_client.CmClient.get_version',
        return_value=Version('7.0.0'),
    ), mock.patch(
        'khulnasoft_checks.cloudera.client.cm_client.CmClient.read_clusters',
        return_value=[{'name': 'cluster_0', 'entity_status': 'GOOD_HEALTH'}],
    ), mock.patch(
        'khulnasoft_checks.cloudera.client.cm_client.CmClient.query_time_series',
        side_effect=query_time_series,
    ), mock.patch(
        'khulnasoft_checks.cloudera.client.cm_client.CmClient.list_hosts',
        side_effect=[list_hosts],
    ), mock.patch(
        'khulnasoft_checks.cloudera.client.cm_client.CmClient.read_events',
        return_value=[],
    ):
        ...
        for expected_metric in expected_metrics:
            for metric in TIMESERIES_METRICS['role']:
                aggregator.assert_metric(
                    f'cloudera.role.{metric}', count=expected_metric.get('count'), tags=expected_metric.get('ts_tags')
                )
