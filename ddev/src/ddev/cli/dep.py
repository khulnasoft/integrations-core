...
IGNORED_DEPS = {
    'ddtrace',  # https://github.com/KhulnaSoft/integrations-core/pull/9132
    'foundationdb',  # Breaking khulnasoft_checks_base tests
...
def read_check_dependencies(repo, integrations=None):
    ...
    for integration in sorted(integrations, key=lambda x: x.name):
        if integration.name in {'khulnasoft_checks_dev', 'khulnasoft_checks_tests_helper', 'ddev'}:
            continue
