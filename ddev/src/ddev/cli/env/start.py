...
@click.option(
    ...
    'extra_env_vars',
    multiple=True,
    help='Environment variables to pass to the Agent e.g. -e DD_URL=app.khulnasoft.com -e DD_API_KEY=foobar',
)
...
def start(
    ...
    *,
    ...
    from khulnasoft_checks.dev._env import deserialize_data
    ...
    if local_base:
        ...
        if (base_package_path := (app.repo.path / 'khulnasoft_checks_base')).is_dir():
            local_packages[base_package_path] = features
        else:
            for repo_name, repo_path in app.config.repos.items():
                if repo_name == app.repo.name:
                    continue
                elif (base_package_path := (Path(repo_path).expand() / 'khulnasoft_checks_base')).is_dir():
                    local_packages[base_package_path] = features
                    break
            else:
