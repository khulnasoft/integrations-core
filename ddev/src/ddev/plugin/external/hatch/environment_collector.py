...
class DatadogChecksEnvironmentCollector(EnvironmentCollectorInterface):
    ...
    def in_core_repo(self):
        return (self.root.parent / 'khulnasoft_checks_base').is_dir()

    @cached_property
    def is_base_package(self):
        return self.root.name == 'khulnasoft_checks_base'

    @cached_property
    def is_test_package(self):
        return self.root.name == 'khulnasoft_checks_dev'
    ...
    def package_directory(self):
        ...
        if name == 'khulnasoft_checks_base':
            directory = 'base'
        elif name == 'khulnasoft_checks_dev':
            directory = 'dev'
        elif name == 'khulnasoft_checks_downloader':
            directory = 'downloader'
        else:
            directory = name.replace('-', '_')

        return f'khulnasoft_checks/{directory}'
    ...
    def get_initial_config(self):
        ...
        lint_env = {
            ...
            'dependencies': [
                'black==22.12.0',
                'ruff==0.0.257',
                # Keep in sync with: /khulnasoft_checks_base/pyproject.toml
                'pydantic==2.0.2',
            ],
        }
