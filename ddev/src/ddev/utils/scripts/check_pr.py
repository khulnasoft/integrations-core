...
# !!!!
# Keep this in sync with khulnasoft_checks_dev/khulnasoft_checks/dev/tooling/constants.py
# !!!!
VALID_CHANGE_TYPES = [
    'added',
    'changed',
...
def requires_changelog(target: str, files: Iterator[str]) -> bool:
    if target == 'ddev':
        ...
        if target == 'khulnasoft_checks_base':
            directory = 'base'
        elif target == 'khulnasoft_checks_dev':
            directory = 'dev'
        elif target == 'khulnasoft_checks_downloader':
            directory = 'downloader'
        else:
            directory = target.replace('-', '_')

        source = f'khulnasoft_checks/{directory}/'
...
def changelog_impl(*, ref: str, diff_file: str, pr_file: str, private: bool, repo: str) -> None:
    ...
    if on_ci:
        ...
        pr_number = 1
        pr_url = f'https://github.com/KhulnaSoft/integrations-core/pull/{pr_number}'
        pr_labels = []
