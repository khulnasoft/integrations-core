...
SKIPPED_PATTERN = re.compile(
    r"""
    khulnasoft_checks_base/khulnasoft_checks/.+
  | khulnasoft_checks_dev/khulnasoft_checks/dev/[^/]+\.py
    """,
    re.VERBOSE,
)
TESTABLE_FILE_PATTERN = re.compile(
  ...
  | metadata\.csv
  | pyproject\.toml
  | khulnasoft_checks/[^/]+/data/metrics\.yaml
  | khulnasoft_checks/snmp/data/default_profiles/.+
    """,
    re.VERBOSE,
)
AGENT_REQUIREMENTS_FILE = 'khulnasoft_checks_base/khulnasoft_checks/base/data/agent_requirements.in'
...
DISPLAY_ORDER_OVERRIDE = {
    ...
    for _i, _d in enumerate(
        (
            'ddev',
            'khulnasoft_checks_base',
            'khulnasoft_checks_dev',
            'khulnasoft_checks_downloader',
        )
    )
}
...
def get_changed_targets(root: Path, *, ref: str, local: bool, verbose: bool) -> list[str]:
    ...
    if (
        (root / 'khulnasoft_checks_base').is_dir()
