...
def test_freeze(ddev, fake_repo):
    ...
    # khulnasoft_checks_dev deps are not shipped with the agent
    create_integration(fake_repo, 'khulnasoft_checks_dev', ['dep-d==4.4.4'])

    result = ddev('dep', 'freeze')

    agent_requirements_path = (
        fake_repo / 'khulnasoft_checks_base' / 'khulnasoft_checks' / 'base' / 'data' / 'agent_requirements.in'
    )
...
def test_sync(ddev, fake_repo):
    create_integration(fake_repo, 'foo', ['dep-a==1.0.0', 'dep-b==3.1.4'])
    create_integration(fake_repo, 'bar', ['dep-a==1.0.0'])
    create_integration(fake_repo, 'khulnasoft_checks_base', ['dep-a==1.0.0'])
    create_integration(fake_repo, 'khulnasoft_checks_downloader', ['dep-a==1.0.0'])

    requirements = """
dep-a==1.1.1
dep-b==3.1.4
"""
    (fake_repo / 'khulnasoft_checks_base' / 'khulnasoft_checks' / 'base' / 'data' / 'agent_requirements.in').write_text(
        requirements.strip('\n')
    )
    ...
    assert_dependencies(fake_repo, 'khulnasoft_checks_base', ['dep-a==1.1.1'])
    assert_dependencies(fake_repo, 'khulnasoft_checks_base', ['dep-a==1.1.1'])
...
class TestUpdates:
    ...
    def requirements_path(self):
        return self.repo / 'khulnasoft_checks_base' / 'khulnasoft_checks' / 'base' / 'data' / 'agent_requirements.in'
...
'''
    ...
    def test_sync(self, ddev):
        self.add_integration('foo', ['dep-a==1.0.0', 'dep-b==3.1.4'])
        self.add_integration('bar', ['dep-a==1.0.0'])
        self.add_integration('khulnasoft_checks_base', ['dep-a==1.0.0', 'dep-b==3.1.4'])
        self.add_integration('khulnasoft_checks_downloader', ['dep-a==1.0.0', 'dep-b==3.1.4'])
        self.write_requirements()
...
'''
        ...
        assert_dependencies(self.repo, 'khulnasoft_checks_base', ['dep-a==1.2.3', 'dep-b==3.1.4'])
        assert_dependencies(self.repo, 'khulnasoft_checks_downloader', ['dep-a==1.2.3', 'dep-b==3.1.4'])
...
def fake_repo(tmp_path, config_file):
    data_folder = tmp_path / 'khulnasoft_checks_base' / 'khulnasoft_checks' / 'base' / 'data'
...
def create_integration(root, name, dependencies):
    ...
    (integration_dir / 'khulnasoft_checks' / name).mkdir(parents=True)
    (integration_dir / 'khulnasoft_checks' / name / '__about__.py').touch()
    (integration_dir / 'khulnasoft_checks' / name / '__init__.py').write_text(
        """
import a
import b
"""
