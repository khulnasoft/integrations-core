...
class TestIsIntegration:
    ...
    def test_not_integration(self, local_repo):
        repo = Repository(local_repo.name, str(local_repo))
        integration = repo.integrations.get('khulnasoft_checks_downloader')
...
class TestIsShippable:
    ...
    def test_excluded(self, local_repo):
        repo = Repository(local_repo.name, str(local_repo))
        integration = repo.integrations.get('khulnasoft_checks_dev')
...
class TestIsAgentCheck:
    ...
    def test_not_check(self, local_repo):
        repo = Repository(local_repo.name, str(local_repo))
        integration = repo.integrations.get('khulnasoft_checks_base')
...
class TestPackageDirectory:
    ...
    def test_base_package(self, local_repo):
        ...
        assert integration.package_directory == local_repo / integration.name / 'khulnasoft_checks' / 'base'

    def test_dev_package(self, local_repo):
        repo = Repository(local_repo.name, str(local_repo))
        integration = repo.integrations.get('khulnasoft_checks_dev')

        assert integration.package_directory == local_repo / integration.name / 'khulnasoft_checks' / 'dev'

    def test_downloader(self, local_repo):
        repo = Repository(local_repo.name, str(local_repo))
        integration = repo.integrations.get('khulnasoft_checks_downloader')

        assert integration.package_directory == local_repo / integration.name / 'khulnasoft_checks' / 'downloader'

    def test_normalization(self, local_repo):
        repo = Repository(local_repo.name, str(local_repo))
        integration = repo.integrations.get('go-metro')

        assert integration.package_directory == local_repo / integration.name / 'khulnasoft_checks' / 'go_metro'
...
class TestReleaseTagPattern:
    def test_shipped(self, local_repo):
        ...
        assert integration.release_tag_pattern == r'khulnasoft_checks_base-\d+\.\d+\.\d+'
