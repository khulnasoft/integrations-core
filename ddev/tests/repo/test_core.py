...
class TestIntegrationsIteration:
    ...
    iter_test_params = [
        ...
        pytest.param(
            "iter_agent_checks",
            lambda path: (
                package_root := path / 'khulnasoft_checks' / path.name.replace('-', '_') / '__init__.py'
            ).is_file()
        ...
        pytest.param(
            "iter_jmx_checks",
            lambda path: (path / 'khulnasoft_checks' / path.name.replace('-', '_') / 'data' / 'metrics.yaml').is_file(),
            id="jmx checks",
        ),
    ]
...
class TestIterationChanged:
    def test(self, repository):
        ...
        (repo.path / 'khulnasoft_checks_base' / 'README.md').remove()
        (repo.path / 'postgres' / 'README.md').remove()

        integrations = list(repo.integrations.iter_changed())
        assert [integration.name for integration in integrations] == ['khulnasoft_checks_base', 'new', 'postgres']
