...
from khulnasoft_checks.base.utils.timeout import TimeoutException
from khulnasoft_checks.dev.utils import get_metadata_metrics
from khulnasoft_checks.disk import Disk
from khulnasoft_checks.disk.disk import IGNORE_CASE
...
def test_device_tagging(aggregator, gauge_metrics, rate_metrics, count_metrics, dd_run_check):
    ...
    with mock.patch('khulnasoft_checks.disk.disk.Disk._get_devices_label'):
...
def test_get_devices_label():
    ...
    with mock.patch(
        "khulnasoft_checks.disk.disk.get_subprocess_output",
...
def test_get_devices_label_options():
    ...
    prefix_fun = "khulnasoft_checks.disk.disk.Disk._get_devices_label_from_"
...
def test_timeout_config(aggregator, dd_run_check):
    ...
    with mock.patch('psutil.disk_partitions', return_value=[MockPart()]), mock.patch(
        'khulnasoft_checks.disk.disk.timeout', return_value=no_timeout
    ) as mock_timeout:
        dd_run_check(c)
...
def test_timeout_warning(aggregator, gauge_metrics, rate_metrics, count_metrics, dd_run_check):
    ...
    ), mock.patch('khulnasoft_checks.disk.disk.timeout', return_value=faulty_timeout):
        dd_run_check(c)
