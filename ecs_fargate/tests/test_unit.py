...
from khulnasoft_checks.ecs_fargate import FargateCheck
...
def test_failing_check(check, aggregator, dd_run_check):
    ...
    """
    with mock.patch(
        'khulnasoft_checks.ecs_fargate.ecs_fargate.requests.get', return_value=MockResponse('{}', status_code=500)
    ):
        dd_run_check(check)
...
def test_invalid_response_check(check, aggregator, dd_run_check):
    ...
    """
    with mock.patch(
        'khulnasoft_checks.ecs_fargate.ecs_fargate.requests.get', return_value=MockResponse('{}', status_code=200)
    ):
        dd_run_check(check)
...
def test_successful_check_linux(check, aggregator, dd_run_check):
    ...
    """
    with mock.patch('khulnasoft_checks.ecs_fargate.ecs_fargate.requests.get', side_effect=mocked_requests_get_linux):
        with mock.patch("khulnasoft_checks.ecs_fargate.ecs_fargate.get_tags", side_effect=mocked_get_tags):
            with mock.patch("khulnasoft_checks.ecs_fargate.ecs_fargate.c_is_excluded", side_effect=mocked_is_excluded):
                dd_run_check(check)
...
def test_successful_check_windows(check, aggregator, dd_run_check):
    ...
    """
    with mock.patch('khulnasoft_checks.ecs_fargate.ecs_fargate.requests.get', side_effect=mocked_requests_get_windows):
        with mock.patch("khulnasoft_checks.ecs_fargate.ecs_fargate.get_tags", side_effect=mocked_get_tags):
            with mock.patch("khulnasoft_checks.ecs_fargate.ecs_fargate.c_is_excluded", side_effect=mocked_is_excluded):
                dd_run_check(check)
...
def test_successful_check_wrong_sys_delta(check, aggregator, dd_run_check):
    ...
    """
    with mock.patch('khulnasoft_checks.ecs_fargate.ecs_fargate.requests.get', side_effect=mocked_requests_get_sys_delta):
        with mock.patch("khulnasoft_checks.ecs_fargate.ecs_fargate.get_tags", side_effect=mocked_get_tags):
            with mock.patch("khulnasoft_checks.ecs_fargate.ecs_fargate.c_is_excluded", side_effect=mocked_is_excluded):
                dd_run_check(check)
...
def test_config(test_case, extra_config, expected_http_kwargs, dd_run_check):
    ...
    with mock.patch('khulnasoft_checks.base.utils.http.requests') as r:
