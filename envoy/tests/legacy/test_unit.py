...
from khulnasoft_checks.envoy import Envoy
from khulnasoft_checks.envoy.metrics import METRIC_PREFIX, METRICS
...
def test_metadata_with_exception(
    khulnasoft_agent, fixture_path, mock_http_response, check, exception, log_call_parameters
):
    instance = INSTANCES['main']
    check = check(instance)
...
def test_metadata(khulnasoft_agent, fixture_path, mock_http_response, check, fixture_file, expected_version):
    instance = INSTANCES['main']
    check = check(instance)
    check.check_id = 'test:123'
    check.log = mock.MagicMock()
...
def test_metadata_invalid(khulnasoft_agent, fixture_path, mock_http_response, check):
    instance = INSTANCES['main']
    check = check(instance)
    check.check_id = 'test:123'
    check.log = mock.MagicMock()
    ...
    khulnasoft_agent.assert_metadata('test:123', {})
    khulnasoft_agent.assert_metadata_count(0)
    check.log.debug.assert_called_with('Version not matched.')


def test_metadata_not_collected(khulnasoft_agent, check):
