...
from khulnasoft_checks.dev.testing import requires_py2, requires_py3
from khulnasoft_checks.dev.utils import get_metadata_metrics
from khulnasoft_checks.envoy.metrics import PROMETHEUS_METRICS_MAP
...
@requires_py3
def test_collect_metadata(khulnasoft_agent, fixture_path, mock_http_response, check, default_instance):
    c = check(default_instance)
    c.check_id = 'test:123'
...
def test_collect_metadata_with_invalid_base_url(
    khulnasoft_agent, fixture_path, mock_http_response, check, default_instance
):
