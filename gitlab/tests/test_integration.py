...
from khulnasoft_checks.gitlab import GitlabCheck
...
def test_check_gitaly(dd_run_check, aggregator, gitlab_check, get_config):
    from khulnasoft_checks.gitlab.gitlab_v2 import GitlabCheckV2
...
def test_check_submit_metadata(
    ...
    with mock.patch('khulnasoft_checks.base.utils.http.requests.Response.json') as g:
        # mock the api call so that it returns the given version
        g.return_value = {"version": raw_version}

        khulnasoft_agent.reset()
        khulnasoft_agent._config["enable_metadata_collection"] = enable_metadata_collection
