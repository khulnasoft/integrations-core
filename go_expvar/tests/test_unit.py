...
MOCK_CONFIG = {
    ...
    "metrics": [
        ...
        {"path": "memstats/PauseTotalNs", "alias": "go_expvar.gc.pause", "type": "rate"},
        {
            "path": "random_walk",
...
CONFIG = {"expvar_url": common.URL_WITH_PATH, 'tags': ['my_tag'], 'metrics': [{'path': 'num_calls', "type": "rate"}]}
...
def test_go_expvar_mocked_namespace(go_expvar_mock, check, aggregator):
    ...
    mock_config = {
        ...
        "metrics": [
            ...
            {"path": "memstats/PauseTotalNs", "alias": "{0}.gc.pause".format(metric_namespace), "type": "rate"},
            {
                "path": "random_walk",
    ...
    for rate in CHECK_RATES:
        aggregator.assert_metric(rate.format(metric_namespace), count=1, tags=shared_tags)
    for rate in CHECK_RATES_CUSTOM_MOCK:
        aggregator.assert_metric(
...
def test_deep_get(go_expvar_mock, check, aggregator):
    ...
    content = {'a': {'one': 1, 'two': 2}, 'b': {'three': 3, 'four': 4}}
    expected = [(['a', 'two'], 2), (['b', 'three'], 3)]
    check.check(MOCK_CONFIG)
    ...
    content = {'list': [{'timestamp': 10, 'value': 5}, {'timestamp': 10, 'value': 10}, {'timestamp': 10, 'value': 20}]}
    expected = [(['list', '0', 'value'], 5), (['list', '1', 'value'], 10), (['list', '2', 'value'], 20)]
...
def test_alias_tag_path(go_expvar_mock, check, aggregator):
    mock_config = {
        "expvar_url": common.URL_WITH_PATH,
        "metrics": [{"path": r"array/\d+/key", "alias": "array.dict.key", "type": "gauge"}],
    }
    check.check(mock_config)
