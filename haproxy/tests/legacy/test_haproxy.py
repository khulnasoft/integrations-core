...
from khulnasoft_checks.haproxy import HAProxyCheck
...
@pytest.mark.usefixtures('dd_environment')
@pytest.mark.integration
def test_version_metadata_http(check, khulnasoft_agent, version_metadata):
    ...
    metadata_count = (
        ...
        if ('test:123', 'version.release') in khulnasoft_agent._metadata
        else len(version_metadata)
    )
    khulnasoft_agent.assert_metadata_count(metadata_count)
...
@pytest.mark.usefixtures('dd_environment')
@pytest.mark.integration
def test_version_metadata_unix_socket(check, version_metadata, dd_environment, khulnasoft_agent):
    config = copy.deepcopy(CONFIG_UNIXSOCKET)
    unixsocket_url = dd_environment['instances'][0]["unixsocket_url"]
    config['url'] = unixsocket_url
    check = check(config)
...
@pytest.mark.usefixtures('dd_environment')
@pytest.mark.integration
def test_version_metadata_tcp_socket(check, version_metadata, khulnasoft_agent):
    config = copy.deepcopy(CONFIG_TCPSOCKET)
    check = check(config)
    check.check_id = 'test:123'
    check.check(config)
