...
def test_version_failure(aggregator, check, khulnasoft_agent):
    config = copy.deepcopy(BASE_CONFIG)
    haproxy_check = check(config)
...
def test_enterprise_version_collection(khulnasoft_agent, check, haproxy_mock_enterprise_version_info):
    config = copy.deepcopy(BASE_CONFIG)
    ...
    khulnasoft_agent.assert_metadata('test:123', expected_version_metadata)
