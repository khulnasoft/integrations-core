...
from khulnasoft_checks.base.utils.time import ensure_aware_datetime
...
def test_no_msg_errors_are_caught(get_check, instance, caplog, dd_run_check):
    ...
    m = mock.MagicMock()
    with mock.patch('khulnasoft_checks.ibm_mq.collectors.channel_metric_collector.pymqi.PCFExecute', new=m), mock.patch(
        'khulnasoft_checks.ibm_mq.collectors.queue_metric_collector.pymqi.PCFExecute', new=m
    ), mock.patch('khulnasoft_checks.ibm_mq.collectors.stats_collector.pymqi.PCFExecute', new=m):
...
def test_check_cant_connect(aggregator, get_check, instance, dd_run_check):
    # Late import to ignore missing library for e2e
    from khulnasoft_checks.ibm_mq import IbmMqCheck
    from khulnasoft_checks.ibm_mq.collectors import QueueMetricCollector
...
def test_check_metrics_and_service_checks(aggregator, get_check, instance, seed_data, override_hostname, dd_run_check):
    # Late import to ignore missing library for e2e
    from khulnasoft_checks.ibm_mq.collectors import ChannelMetricCollector, QueueMetricCollector
...
def test_stats_metrics(aggregator, get_check, instance, dd_run_check):
    ...
    with open(os.path.join(common.HERE, 'fixtures', 'statistics_channel.data'), 'rb') as channel_file, open(
        ...
        with mock.patch('khulnasoft_checks.ibm_mq.collectors.stats_collector.Queue') as queue:
