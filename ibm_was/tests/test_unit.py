...
from khulnasoft_checks.ibm_was import IbmWasCheck
...
def test_metric_collection_per_category(aggregator, instance, check):
    with mock.patch('khulnasoft_checks.ibm_was.IbmWasCheck.make_request', return_value=mock_data('server.xml')):
...
def test_right_server_tag(instance, check, aggregator):
    ...
    with mock.patch(
        'khulnasoft_checks.ibm_was.IbmWasCheck.make_request', return_value=mock_data('perfservlet-multiple-nodes.xml')
    ):
        check = check(instance)
        check.check(instance)
