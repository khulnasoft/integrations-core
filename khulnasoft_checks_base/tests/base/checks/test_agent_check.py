...
from khulnasoft_checks.base import AgentCheck, to_native_string
from khulnasoft_checks.base import __version__ as base_package_version
...
def test_persistent_cache(khulnasoft_agent):
    check = AgentCheck()
    check.check_id = 'test'

    check.write_persistent_cache('foo', 'bar')

    assert khulnasoft_agent.read_persistent_cache('test_foo') == 'bar'
...
def test_is_metadata_collection_enabled(enable_metadata_collection, expected_is_metadata_collection_enabled):
    check = AgentCheck()
    with mock.patch('khulnasoft_checks.base.checks.base.khulnasoft_agent.get_config') as get_config:
...
class TestTags:
    ...
    def test_external_hostname(self, khulnasoft_agent):
        check = AgentCheck()
        external_host_tags = [(u'hostnam\xe9', {'src_name': ['key1:val1']})]
        check.set_external_tags(external_host_tags)

        if PY3:
            khulnasoft_agent.assert_external_tags(u'hostnam\xe9', {'src_name': ['key1:val1']})
        else:
            khulnasoft_agent.assert_external_tags('hostnam\xc3\xa9', {'src_name': ['key1:val1']})

        khulnasoft_agent.assert_external_tags_count(1)
...
def test_load_configuration_models(dd_run_check, mocker):
    ...
    import_module.assert_called_with('khulnasoft_checks.base.config_models')
