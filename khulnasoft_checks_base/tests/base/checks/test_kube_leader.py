...
from khulnasoft_checks.base import AgentCheck, KubeLeaderElectionBaseCheck
from khulnasoft_checks.base.checks.kube_leader import ElectionRecordAnnotation, ElectionRecordLease
...
def mock_incluster():
    ...
    with mock.patch('khulnasoft_checks.base.checks.kube_leader.mixins.config.load_incluster_config'):
        yield
...
def mock_read_endpoints():
    ...
    with mock.patch('khulnasoft_checks.base.checks.kube_leader.mixins.client.CoreV1Api.read_namespaced_endpoints') as m:
        yield m
...
def mock_read_configmap():
    ...
    with mock.patch('khulnasoft_checks.base.checks.kube_leader.mixins.client.CoreV1Api.read_namespaced_config_map') as m:
        yield m
...
@mock.patch('khulnasoft_checks.base.checks.kube_leader.mixins.config')
class TestClientConfig:
    ...
    @mock.patch('khulnasoft_checks.base.checks.kube_leader.mixins.khulnasoft_agent')
    def test_config_kubeconfig(self, khulnasoft_agent, config):
        khulnasoft_agent.get_config.return_value = "/file/path"
        c = KubeLeaderElectionBaseCheck()
        c.check({})
        khulnasoft_agent.get_config.assert_called_once_with('kubernetes_kubeconfig_path')
