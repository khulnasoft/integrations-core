...
def test_assert_external_tags(khulnasoft_agent, hostname, tags, raise_exception, match_tags_order):
    khulnasoft_agent.set_external_tags(DEFAULT_EXTERNAL_TAGS)

    try:
        khulnasoft_agent.assert_external_tags(hostname, tags, match_tags_order)
...
def test_assert_external_tags_count(khulnasoft_agent, external_tags, count, raise_exception):
    khulnasoft_agent.set_external_tags(external_tags)

    try:
        khulnasoft_agent.assert_external_tags_count(count)
    except AssertionError:
        if not raise_exception:
            raise
