...
@pytest.fixture
def khulnasoft_agent():
    global __khulnasoft_agent

    if __khulnasoft_agent is None:
        try:
...
@pytest.fixture
def dd_agent_check(request, aggregator, khulnasoft_agent):
    if not e2e_testing():
        pytest.skip('Not running E2E tests')

    # Lazily import to reduce plugin load times for everyone
    from khulnasoft_checks.dev import TempDir, run_command
    ...
    def run_check(config=None, **kwargs):
        ...
        for raw_json in matches:
            ...
            replay_check_run(collector, aggregator, khulnasoft_agent)
...
def mock_performance_objects(mocker, dd_default_hostname):
    def mock_perf_objects(
        ...
        validate_path_fn_name = 'khulnasoft_checks.base.checks.windows.perf_counters.counter.validate_path'
        ...
        get_counter_values_fn_name = 'khulnasoft_checks.base.checks.windows.perf_counters.counter.get_counter_values'
