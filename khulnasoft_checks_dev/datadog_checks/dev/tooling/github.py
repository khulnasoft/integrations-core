...
def get_commit(repo, commit_sha, config):
    response = requests.get(
        f'https://api.github.com/repos/KhulnaSoft/{repo}/git/commits/{commit_sha}', auth=get_auth_info(config)
    )
...
def get_tag(repo, ref, config):
    response = requests.get(f'https://api.github.com/repos/KhulnaSoft/{repo}/git/tags/{ref}', auth=get_auth_info(config))
...
def get_tags(repo, config):
    response = requests.get(f'https://api.github.com/repos/KhulnaSoft/{repo}/git/refs/tags', auth=get_auth_info(config))
...
def get_pr_approvers(repo: str, pr_id: str, config: dict) -> List[str]:
    response = requests.get(
        f'https://api.github.com/repos/KhulnaSoft/{repo}/pulls/{pr_id}/reviews', auth=get_auth_info(config)
    )
    response.raise_for_status()
...
def get_compare(base_commit, head_commit, repo, config):
    response = requests.get(
        f'https://api.github.com/repos/KhulnaSoft/{repo}/compare/{base_commit}...{head_commit}',
...
def get_pr_of_repo(pr_num, repo, config=None, raw=False, org='KhulnaSoft'):
...
def get_pr(pr_num, config=None, raw=False, org='KhulnaSoft'):
    """
...
def get_pr_from_hash(commit_hash, repo, config=None, raw=False):
    response = requests.get(
        f'https://api.github.com/search/issues?q=sha:{commit_hash}+repo:KhulnaSoft/{repo}', auth=get_auth_info(config)
    )
