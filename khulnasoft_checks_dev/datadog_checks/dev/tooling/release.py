...
def get_package_name(folder_name):
    ...
    return f"{KHULNASOFT_PACKAGE_PREFIX}{folder_name.replace('_', '-')}"
...
def get_folder_name(package_name):
    ...
    return package_name.replace('-', '_')[len(KHULNASOFT_PACKAGE_PREFIX) :]
...
def get_agent_requirement_line(check, version):
    ...
    # no manifest
    if check in ('khulnasoft_checks_base', 'khulnasoft_checks_downloader', 'khulnasoft_checks_dependency_provider', 'ddev'):
