...
def prepare_tox_test_commands(
    ...
    base_or_dev = check.startswith('khulnasoft_checks_')
    if force_base_min and not base_or_dev:
        ...
        if spec is None:
            abort(f'\nFailed to determine minimum version of package `khulnasoft_checks_base`: {spec}')

        version = spec.split('>=')[-1]

        env_vars['TOX_FORCE_INSTALL'] = f'khulnasoft_checks_base[deps]=={version}'
        env_vars['BASE_PACKAGE_FORCE_VERSION'] = str(version)
    elif force_base_unpinned and not base_or_dev:
        env_vars['TOX_FORCE_UNPINNED'] = 'khulnasoft_checks_base'
...
def prepare_hatch_test_commands(
    ...
    base_or_dev = check.startswith('khulnasoft_checks_') or check == 'ddev'
    if force_base_min and not base_or_dev:
        check_base_dependencies, errors = read_check_base_dependencies(check)
        if errors:
            abort(f'\nError collecting base package dependencies: {errors}')
...
def coverage_sources(check):
    ...
    if check == 'khulnasoft_checks_base':
        package_path = 'khulnasoft_checks/base'
    elif check == 'khulnasoft_checks_dev':
        package_path = 'khulnasoft_checks/dev'
    elif check == 'khulnasoft_checks_downloader':
        package_path = 'khulnasoft_checks/downloader'
    else:
        package_path = f'khulnasoft_checks/{check}'
...
def process_checks_option(check, source=None, validate=False, extend_changed=False):
    ...
    if check is None or check.lower() == 'all':
        ...
        if extend_changed and ('khulnasoft_checks_dev' in choice or 'khulnasoft_checks_base' in choice):
            choice = sorted(get_valid())
    else:
        if validate:
