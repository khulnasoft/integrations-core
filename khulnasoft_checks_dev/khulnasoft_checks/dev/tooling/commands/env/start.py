...
@click.option(
    ...
    help=(
        ...
        'Ex: -e DD_URL=app.khulnasoft.com -e DD_API_KEY=123456'
    ),
)
...
def _get_base_package(base, ctx):
    ...
    if base:
        ...
        base_package = path_join(core_dir, 'khulnasoft_checks_base')
        if not dir_exists(base_package):
            abort('`khulnasoft_checks_base` directory does not exist.')
    return base_package
...
def _get_env_vars(env_vars, metadata, dogstatsd, profile_memory, check, on_ci):
    ...
    for key, value in metadata.get('env_vars', {}).items():
        ...
        # https://github.com/KhulnaSoft/integrations-core/pull/5346
