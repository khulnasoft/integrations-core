...
def validate_import(filepath, check, autofix):
    ...
    # almost every case is of the form `from khulnasoft_checks.. import ..`
    # we want to ensure that the imports are from `khulnasoft_checks.base...`
    ...
    with open(filepath) as f:
        for num, line in enumerate(f):
            if all(
                (
                    'import' in line,
                    'khulnasoft_checks' in line,
                    'khulnasoft_checks.base' not in line,
                    f'khulnasoft_checks.{check}' not in line,
                    'khulnasoft_checks.dev' not in line,
                )
            ):
                success = False
    ...
    if autofix and not success:
        ...
        for num, _ in lines:
            data[num] = data[num].replace('khulnasoft_checks', 'khulnasoft_checks.base')
