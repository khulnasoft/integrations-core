...
def get_agent_exe(agent_version, platform=LINUX):
    if platform == WINDOWS:
        if agent_version >= 6:
            return r'C:\Program Files\Khulnasoft\Khulnasoft Agent\bin\agent.exe'
        else:
        ...
        if agent_version >= 6:
            return '/opt/khulnasoft-agent/bin/agent/agent'
        else:
            return '/opt/khulnasoft-agent/agent/agent.py'


def get_python_exe(platform=LINUX, python_version=DEFAULT_PYTHON_VERSION):
    if platform == WINDOWS:
        return r'C:\Program Files\Khulnasoft\Khulnasoft Agent\embedded{}\python.exe'.format(python_version)
    else:
        return f'/opt/khulnasoft-agent/embedded/bin/python{python_version}'
...
def get_pip_exe(python_version, platform=LINUX):
    if platform == WINDOWS:
        ...
        return [f'/opt/khulnasoft-agent/embedded/bin/pip{python_version}']


def get_agent_conf_dir(check, agent_version, platform=LINUX):
    if platform == WINDOWS:
        if agent_version >= 6:
            return r'C:\ProgramData\Khulnasoft\conf.d\{}.d'.format(check)
        else:
            return r'C:\ProgramData\Khulnasoft\conf.d'
    elif platform == MAC:
        if agent_version >= 6:
            return f'/opt/khulnasoft-agent/etc/conf.d/{check}.d'
        else:
            return '/opt/khulnasoft-agent/etc/conf.d'
    else:
        if agent_version >= 6:
            return f'/etc/khulnasoft-agent/conf.d/{check}.d'
        else:
...
def get_agent_version_manifest(platform):
    if platform == WINDOWS:
        return r'C:\Program Files\Khulnasoft\Khulnasoft Agent\version-manifest.txt'
    else:
        return '/opt/khulnasoft-agent/version-manifest.txt'
...
def get_agent_service_cmd(version, platform, action):
    if platform == WINDOWS:
        ...
        return (
            r'powershell -executionpolicy bypass -Command Start-Process """""""""C:\Program Files\Khulnasoft\Khulnasoft '
