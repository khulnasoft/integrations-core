...
def get_check_file(check_name):
    return os.path.join(get_root(), check_name, 'khulnasoft_checks', check_name, check_name + '.py')
...
def get_package_name(check_name):
    if check_name == 'khulnasoft_checks_base':
        return 'base'
    elif check_name == 'khulnasoft_checks_dev':
        return 'dev'
    elif check_name == 'khulnasoft_checks_downloader':
        return 'downloader'
    else:
...
def get_version_file(check_name):
    if check_name == 'ddev':
        ...
        return os.path.join(get_root(), check_name, 'khulnasoft_checks', get_package_name(check_name), '__about__.py')


def is_agent_check(check_name):
    package_root = os.path.join(get_root(), check_name, 'khulnasoft_checks', check_name, '__init__.py')
...
def get_jmx_metrics_file(check_name):
    path = os.path.join(get_root(), check_name, 'khulnasoft_checks', check_name, 'data', 'metrics.yaml')
...
def get_data_directory(check_name):
    if check_name == 'agent':
        ...
        return os.path.join(get_root(), check_name, 'khulnasoft_checks', check_name, 'data')


def get_models_location(check_name):
    return os.path.join(get_root(), check_name, 'khulnasoft_checks', check_name, 'config_models')


def get_check_directory(check_name):
    return os.path.join(get_root(), check_name, 'khulnasoft_checks', check_name)
...
def get_config_files(check_name):
    ...
    auto_conf = os.path.join(root, check_name, 'khulnasoft_checks', check_name, 'data', 'auto_conf.yaml')
    if file_exists(auto_conf):
        files.append(auto_conf)

    default_yaml = os.path.join(root, check_name, 'khulnasoft_checks', check_name, 'data', 'conf.yaml.default')
    if file_exists(default_yaml):
        files.append(default_yaml)

    example_yaml = os.path.join(root, check_name, 'khulnasoft_checks', check_name, 'data', 'conf.yaml.example')
...
def get_check_files(check_name, file_suffix='.py', abs_file_path=True, include_tests=True, include_dirs=None):
    ...
    By default, only includes files within 'khulnasoft_checks' and 'tests' directories, this
    can be expanded by adding to the `include_dirs` arg. 'tests' can also be removed.
    """
    base_dirs = ['khulnasoft_checks']
    if include_tests:
