...
class TestKindRun:
    ...
    def test_retry_on_failed_conditions(self, attempts, expected_call_count):
        ...
        with pytest.raises(expected_exception), patch('khulnasoft_checks.dev.kind.KindUp'), patch(
            'khulnasoft_checks.dev.kind.KindDown'
        ):
    ...
    def test_retry_condition_failed_only_on_first_run(self):
        ...
        with patch('khulnasoft_checks.dev.kind.KindUp', return_value=up), patch(
            'khulnasoft_checks.dev.kind.KindDown', return_value=MagicMock()
        ):
