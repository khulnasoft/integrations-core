...
from khulnasoft_checks.dev.tooling.commands.release.make import validate_version
...
def test_release_make_with_invalid_input_version(version):
    result = run_command(
        [sys.executable, '-m', 'khulnasoft_checks.dev', '-x', 'release', 'make', 'my_check', '--version', version],
        capture=True,
    )
