...
from khulnasoft_checks.dev import run_command
from khulnasoft_checks.dev.tooling.configuration.consumers.model.model_consumer import VALIDATORS_DOCUMENTATION
from khulnasoft_checks.dev.tooling.utils import get_license_header
...
def test_generate_new_files_check_licenses(repo, expect_licenses):
    ...
    with runner.isolated_filesystem():
        ...
        result = run_command(
            [sys.executable, '-m', 'khulnasoft_checks.dev', '--here', 'validate', 'models', 'my_check', "-s"],
            capture=True,
        )
        ...
        for filename in os.listdir("my_check/khulnasoft_checks/my_check/config_models"):
            if filename != ".gitkeep":
                with open(f"my_check/khulnasoft_checks/my_check/config_models/{filename}", mode='r') as file:
        ...
        with open("my_check/khulnasoft_checks/my_check/config_models/validators.py", mode='r') as validators_file:
            if expect_licenses:
...
def test_validate_config_models_not_in_sync(repo, expect_failure):
    ...
    with runner.isolated_filesystem():
        ...
        os.chdir(working_repo)
        shutil.rmtree("my_check/khulnasoft_checks/my_check/config_models")

        result = run_command(
            [sys.executable, '-m', 'khulnasoft_checks.dev', '--here', 'validate', 'models', 'my_check'],
            capture=True,
        )
...
def test_validate_no_config_models(repo, expect_failure):
    ...
    with runner.isolated_filesystem():
        ...
        result = run_command(
            [sys.executable, '-m', 'khulnasoft_checks.dev', '--here', 'validate', 'models', 'tokumx'],
            capture=True,
        )
