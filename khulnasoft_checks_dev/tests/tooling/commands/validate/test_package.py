...
def test_validate_package_validates_emails(authors, expected_exit_code, expected_output):
    ...
    with runner.isolated_filesystem():
        ...
        os.makedirs('my_check/khulnasoft_checks/my_check')
        with open('my_check/khulnasoft_checks/my_check/__about__.py', 'w') as f:
            f.write('__version__ = "1.0.0"')

        result = run_command(
            [sys.executable, '-m', 'khulnasoft_checks.dev', '-x', 'validate', 'package', 'my_check'],
            capture=True,
        )
