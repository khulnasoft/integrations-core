...
from khulnasoft_checks.dev.tooling.license_headers import parse_license_header, validate_license_headers
...
def test_validate_license_headers_works_with_arbitrary_nesting(tmp_path):
    ...
    nested_path = check_path / "khulnasoft_checks/check"
    ...
    assert len(errors) == 1
    assert errors[0].message == "missing license header"
    assert errors[0].path == "khulnasoft_checks/check/check.py"
