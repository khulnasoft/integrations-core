...
from khulnasoft_checks.dev.tooling.config import copy_default_config
from khulnasoft_checks.dev.tooling.utils import (
    complete_set_root,
    get_check_files,
    get_version_string,
...
def test_get_version_string():
    with mock.patch('khulnasoft_checks.dev.tooling.utils.read_version_file') as read:
        read.return_value = '__version__ = "2.0.0"'
        assert get_version_string('foo_check') == '2.0.0'


@mock.patch('khulnasoft_checks.dev.tooling.utils.get_root')
...
@mock.patch('khulnasoft_checks.dev.tooling.utils.get_root')
@mock.patch('khulnasoft_checks.dev.tooling.utils.set_root')
...
def test_complete_set_root_no_args(set_root, get_root):
    ...
    with mock.patch('khulnasoft_checks.dev.tooling.utils.load_config') as load_config:
        config = copy_default_config()
...
def test_get_check_files(get_root):
    ...
    mock_dir_map = {
        '': [
            (
                'dns_check',
                ['khulnasoft_checks', 'khulnasoft_dns_check.egg-info', 'tests', '.junit', 'assets'],
                [
                    'CHANGELOG.md',
                    'MANIFEST.in',
                    'setup.py',
        ...
        'khulnasoft_checks': [
            (join('dns_check', 'khulnasoft_checks'), ['dns_check'], ['__init__.py']),
            (
                join('dns_check', 'khulnasoft_checks', 'dns_check'),
                ['data'],
                ['__init__.py', '__about__.py', 'dns_check.py'],
            ),
            (join('dns_check', 'khulnasoft_checks', 'dns_check', 'data'), [], ['conf.yaml.example']),
        ],
        '.tox': [(join('dns_check', '.tox'), ['py37', '.tmp', 'py27'], [])],
        'khulnasoft_dns_check.egg-info': [
            (join('dns_check', 'khulnasoft_dns_check.egg-info'), [], ['PKG-INFO', 'SOURCES.txt'])
        ],
    ...
    default_py_files = [
        join('dns_check', 'khulnasoft_checks', '__init__.py'),
        join('dns_check', 'khulnasoft_checks', 'dns_check', '__init__.py'),
        join('dns_check', 'khulnasoft_checks', 'dns_check', '__about__.py'),
        join('dns_check', 'khulnasoft_checks', 'dns_check', 'dns_check.py'),
