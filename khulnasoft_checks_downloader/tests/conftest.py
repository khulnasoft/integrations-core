...
def temporary_local_repo(monkeypatch, tmp_path):
    ...
    from khulnasoft_checks.downloader.download import REPOSITORIES_DIR, REPOSITORY_DIR

    src_dir = os.path.join(REPOSITORIES_DIR, REPOSITORY_DIR)
    dst_dir = os.path.join(tmp_path, REPOSITORY_DIR)

    shutil.copytree(src_dir, dst_dir)
    monkeypatch.setattr('khulnasoft_checks.downloader.download.REPOSITORIES_DIR', tmp_path)
    yield tmp_path
