...
def modified_args(argv):
    ...
    sys.argv = ["khulnasoft_checks_downloader"] + argv  # Make sure argv[0] (program name) is prepended.
...
def restore_repo_state(tmp_path):
    ...
    src_dir = os.path.join(os.path.dirname(khulnasoft_checks.downloader.__file__), 'data')
...
def _do_download(package, version=None, root_layout_type="core"):
    ...
    - When downloading the first package, the khulnasoft_checks_downloader will also download the signing metadata and
      especially "timestamp.json" which gives the current "version". Anytime a new package is pushed, that version
      gets increased.
    - When downloading package A, khulnasoft_checks_downloader will write to disk the current version of "timestamp.json"
    - When downloading package B, khulnasoft_checks_downloader will check the current version of timestamp.json against
    ...
    Source: https://github.com/KhulnaSoft/integrations-core/pull/6476#issuecomment-619059117
    """
    # -v:     CRITICAL
    # -vv:    ERROR
    # -vvv:   WARNING
