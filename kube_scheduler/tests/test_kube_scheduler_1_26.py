...
from khulnasoft_checks.base.checks.kube_leader import ElectionRecordAnnotation
from khulnasoft_checks.kube_scheduler import KubeSchedulerCheck
...
@pytest.fixture()
def mock_metrics():
    f_name = os.path.join(HERE, 'fixtures', 'metrics_1.26.0.txt')
...
def mock_leader():
    ...
    with mock.patch(
        'khulnasoft_checks.kube_scheduler.KubeSchedulerCheck._get_record',
...
def test_check_metrics_1_26(aggregator, mock_metrics, mock_leader, instance):
    ...
    assert_metric(
        ...
        tags=['host:192.168.58.2:8443', 'upper_bound:8.0', 'verb:GET'],
    )
    assert_metric(
        'client.http.requests_duration.sum',
        value=0.7267887909999999,
        tags=['host:192.168.58.2:8443', 'verb:GET'],
    )
