...
from khulnasoft_checks.base.checks.kubelet_base.base import KubeletCredentials
from khulnasoft_checks.base.utils.date import parse_rfc3339
from khulnasoft_checks.dev.http import MockResponse
from khulnasoft_checks.kubelet import KubeletCheck, PodListUtils
...
@pytest.fixture
def tagger():
    from khulnasoft_checks.base.stubs import tagger
...
def test_kubelet_credentials_update(monkeypatch, aggregator):
    ...
    with mock.patch('requests.get', return_value=get), mock.patch(
        'khulnasoft_checks.kubelet.kubelet.get_connection_info', return_value=kubelet_conn_info
    ):
        check.check(instance)
...
def test_prometheus_filtering(monkeypatch, aggregator):
    ...
    method_name = "khulnasoft_checks.kubelet.prometheus.CadvisorPrometheusScraperMixin.container_cpu_usage_seconds_total"
...
def test_kubelet_unavailable_check_can_init(monkeypatch):
    ...
    kubelet_conn_info = {}
    with mock.patch('khulnasoft_checks.kubelet.kubelet.get_connection_info', return_value=kubelet_conn_info):
        check = mock_kubelet_check(monkeypatch, [instance], kube_version=None)
