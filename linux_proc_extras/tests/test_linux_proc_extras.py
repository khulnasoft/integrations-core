...
def test_check(aggregator, check):
    ...
    with open(os.path.join(common.FIXTURE_DIR, "entropy_avail")) as f:
        ...
        with patch('khulnasoft_checks.linux_proc_extras.linux_proc_extras.open', m):
    ...
    with open(os.path.join(common.FIXTURE_DIR, "process_stats")) as f:
        with patch(
            'khulnasoft_checks.linux_proc_extras.linux_proc_extras.get_subprocess_output', return_value=(f.read(), "", 0)
        ):
