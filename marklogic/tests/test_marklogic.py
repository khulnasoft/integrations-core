...
@pytest.mark.integration
@pytest.mark.usefixtures('dd_environment')
def test_metadata_integration(aggregator, khulnasoft_agent):
...
def test_submit_health_service_checks(aggregator, caplog):
    ...
    with mock.patch('khulnasoft_checks.marklogic.api.MarkLogicApi.get_health', return_value=health_mocked_data):
    ...
    with mock.patch(
        'khulnasoft_checks.marklogic.api.MarkLogicApi.get_health', return_value={'code': 'HEALTH-CLUSTER-ERROR'}
    ):
        check.submit_health_service_checks()
    ...
    with mock.patch('khulnasoft_checks.marklogic.api.MarkLogicApi.get_health', side_effect=Exception("exception")):
        check.submit_health_service_checks()
