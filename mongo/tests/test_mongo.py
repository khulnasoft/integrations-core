...
from khulnasoft_checks.mongo.common import ReplicaSetDeployment
...
@common.standalone
def test_metadata(check, instance, khulnasoft_agent):
    check = check(instance)
    ...
    khulnasoft_agent.assert_metadata_count(len(version_metadata) + 2)
...
def test_refresh_role(instance_shard, aggregator, check, dd_run_check):
    mongo_check = check(instance_shard)
    dd_run_check(mongo_check)
    with mock.patch('khulnasoft_checks.mongo.api.MongoApi._get_rs_deployment_from_status_payload') as get_deployment:
