...
from khulnasoft_checks.mongo import MongoDb, metrics
from khulnasoft_checks.mongo.api import CRITICAL_FAILURE, MongoApi
from khulnasoft_checks.mongo.collectors import MongoCollector
from khulnasoft_checks.mongo.common import MongosDeployment, ReplicaSetDeployment, get_state_name
from khulnasoft_checks.mongo.config import MongoConfig
from khulnasoft_checks.mongo.utils import parse_mongo_uri
...
def test_emits_ok_service_check_when_service_is_available(
    mock_list_database_names, mock_server_info, mock_command, dd_run_check, aggregator, khulnasoft_agent
):
    # Given
    check = MongoDb('mongo', {}, [{'hosts': ['localhost']}])
...
def test_version_metadata(
    ...
    dd_run_check(check)
    # Then
    khulnasoft_agent.assert_metadata(
        'test:123',
        {
...
def test_api_alibaba_mongos(aggregator):
    ...
    with mock.patch('khulnasoft_checks.mongo.api.MongoClient', mock.MagicMock(return_value=mocked_client)):
...
def test_parse_mongo_version_with_suffix(check, instance, dd_run_check, khulnasoft_agent):
    '''
    ...
    with mock_pymongo('standalone') as mocked_client:
        ...
        dd_run_check(check)
    khulnasoft_agent.assert_metadata('test:123', {'version.scheme': 'semver', 'version.major': '3', 'version.minor': '6'})
