...
from khulnasoft_checks.base import is_affirmative
from khulnasoft_checks.base.utils.common import to_native_string
from khulnasoft_checks.base.utils.db.sql import compute_exec_plan_signature, compute_sql_signature
...
from khulnasoft_checks.base.utils.tracking import tracked_method
...
class MySQLStatementSamples(DBMAsyncJob):
    ...
    def _collect_plan_for_statement(self, row):
        ...
        if plan:
            try:
                normalized_plan = khulnasoft_agent.obfuscate_sql_exec_plan(plan, normalize=True) if plan else None
                obfuscated_plan = khulnasoft_agent.obfuscate_sql_exec_plan(plan)
            except Exception as e:
        ...
        if self._seen_samples_ratelimiter.acquire(query_plan_cache_key):
            return {
                ...
                "ddagentversion": khulnasoft_agent.get_version(),
                "ddsource": "mysql",
        ...
        if not enabled_consumers:
            self._check.record_warning(
                ...
                warning_with_tags(
                    ...
                    'to collect statement samples. See https://docs.khulnasoft.com/database_monitoring/setup_mysql/'
            ...
            if e.args[0] in PYMYSQL_MISSING_EXPLAIN_STATEMENT_PROC_ERRORS:
                ...
                self._check.record_warning(
                    ...
                    warning_with_tags(
                        ...
                        "granted access to in schema '%s'. See https://docs.khulnasoft.com/database_monitoring/"
            ...
            if e.args[0] in PYMYSQL_MISSING_EXPLAIN_STATEMENT_PROC_ERRORS:
                ...
                self._check.record_warning(
                    ...
                    warning_with_tags(
                        ...
                        'not granted access to. See https://docs.khulnasoft.com/database_monitoring/setup_mysql/'
