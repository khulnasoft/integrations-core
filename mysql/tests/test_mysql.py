...
from khulnasoft_checks.mysql.__about__ import __version__
from khulnasoft_checks.mysql.const import (
    BINLOG_VARS,
    GALERA_VARS,
...
def test_correct_hostname(dbm_enabled, reported_hostname, expected_hostname, aggregator, dd_run_check, instance_basic):
    ...
    with mock.patch('khulnasoft_checks.mysql.MySql.resolve_db_host', return_value='resolved.hostname') as resolve_db_host:
        mysql_check = MySql(common.CHECK_NAME, {}, [instance_basic])
        dd_run_check(mysql_check)
...
@pytest.mark.integration
@pytest.mark.usefixtures('dd_environment')
def test_version_metadata(dd_run_check, instance_basic, khulnasoft_agent, version_metadata):
    mysql_check = MySql(common.CHECK_NAME, {}, [instance_basic])
