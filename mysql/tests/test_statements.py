...
from khulnasoft_checks.mysql import MySql, statements
from khulnasoft_checks.mysql.statement_samples import StatementTruncationState
...
def test_statement_metrics(
    aggregator, dd_run_check, dbm_instance, query, default_schema, khulnasoft_agent, aurora_replication_role
):
    ...
    with mock.patch.object(khulnasoft_agent, 'obfuscate_sql', passthrough=True) as m_obfuscate_sql, mock.patch.object(
...
@pytest.mark.usefixtures('dd_environment')
@mock.patch.dict('os.environ', {'DDEV_SKIP_GENERIC_TAGS_CHECK': 'true'})
def test_statement_metrics_with_duplicates(aggregator, dd_run_check, dbm_instance, khulnasoft_agent):
...
def test_statement_metrics_cloud_metadata(
    aggregator, dd_run_check, dbm_instance, input_cloud_metadata, output_cloud_metadata, khulnasoft_agent
):
...
def test_statement_samples_collect(
    ...
    caplog.set_level(logging.INFO, logger="khulnasoft_checks.mysql.collection_utils")
    caplog.set_level(logging.DEBUG, logger="khulnasoft_checks")
    ...
    [
        (
            ...
            [
                ...
                "See https://docs.khulnasoft.com/database_monitoring/setup_mysql/troubleshooting#"
...
def test_performance_schema_disabled(dbm_instance, dd_run_check):
    ...
    assert mysql_check.warnings == [
        ...
        'https://docs.khulnasoft.com/database_monitoring/setup_mysql/troubleshooting#performance-schema-not-enabled '
        'for more details\n'
...
def test_statement_metadata(
    aggregator, dd_run_check, dbm_instance, khulnasoft_agent, metadata, expected_metadata_payload, root_conn
):
    mysql_check = MySql(common.CHECK_NAME, {}, [dbm_instance])
...
def test_statement_reported_hostname(
    aggregator, dd_run_check, dbm_instance, khulnasoft_agent, reported_hostname, expected_hostname
):
