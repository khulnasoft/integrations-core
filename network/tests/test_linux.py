...
from khulnasoft_checks.base.utils.platform import Platform
from khulnasoft_checks.base.utils.subprocess_output import get_subprocess_output
from khulnasoft_checks.dev.utils import get_metadata_metrics
from khulnasoft_checks.network.check_linux import LinuxNetwork
...
@mock.patch('khulnasoft_checks.network.network.Platform.is_linux', return_value=True)
...
def test_collect_cx_queues_when_ss_fails(check, aggregator):
    ...
    instance['collect_connection_state'] = True
    check_instance = LinuxNetwork('network', {}, [instance])
    with mock.patch('khulnasoft_checks.network.check_linux.get_subprocess_output') as out:
...
@mock.patch('khulnasoft_checks.network.check_linux.LinuxNetwork._read_int_file', side_effect=read_int_file_mock)
...
def test_add_conntrack_stats_metrics(aggregator):
    ...
    with mock.patch('khulnasoft_checks.network.check_linux.get_subprocess_output') as subprocess:
...
@mock.patch('khulnasoft_checks.network.network.find_executable', return_value='/bin/ss')
def test_ss_with_custom_procfs(aggregator):
    instance = copy.deepcopy(common.INSTANCE)
    instance['collect_connection_state'] = True
    check_instance = LinuxNetwork('network', {}, [instance])
    with mock.patch(
        'khulnasoft_checks.network.check_linux.get_subprocess_output', side_effect=ss_subprocess_mock
