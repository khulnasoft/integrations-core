...
from khulnasoft_checks.network.check_solaris import SolarisNetwork

from . import common
from .common import FIXTURE_DIR, decode_string


@mock.patch('khulnasoft_checks.network.network.Platform.is_windows', return_value=False)
@mock.patch('khulnasoft_checks.network.network.Platform.is_linux', return_value=False)
@mock.patch('khulnasoft_checks.network.network.Platform.is_bsd', return_value=False)
@mock.patch('khulnasoft_checks.network.network.Platform.is_solaris', return_value=True)
...
def test_check_solaris(instance, aggregator):
    ...
    with mock.patch('khulnasoft_checks.network.check_solaris.get_subprocess_output') as out:
