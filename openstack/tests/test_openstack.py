...
from khulnasoft_checks.openstack.openstack import (
    IncompleteAuthScope,
    IncompleteConfig,
    IncompleteIdentity,
    KeystoneCatalog,
...
def test_from_config():
    ...
    with mock.patch(
        'khulnasoft_checks.openstack.openstack.OpenStackProjectScope.request_auth_token', return_value=MOCK_HTTP_RESPONSE
    ):
...
def test_unscoped_from_config():
    ...
    with mock.patch(
        'khulnasoft_checks.openstack.openstack.OpenStackUnscoped.request_auth_token', return_value=mock_response
    ):
        with mock.patch(
            'khulnasoft_checks.openstack.openstack.OpenStackUnscoped.request_project_list',
            return_value=MOCK_HTTP_PROJECTS_RESPONSE,
        ):
            with mock.patch(
                'khulnasoft_checks.openstack.openstack.OpenStackUnscoped.get_token_for_project',
...
def test_cache_utils():
    ...
    with mock.patch(
        'khulnasoft_checks.openstack.OpenStackCheck.get_all_aggregate_hypervisors', return_value=expected_aggregates
    ):
...
@mock.patch('khulnasoft_checks.openstack.OpenStackCheck.get_all_servers', return_value=common.ALL_SERVER_DETAILS)
...
@mock.patch('khulnasoft_checks.openstack.OpenStackCheck.get_all_network_ids', return_value=common.ALL_IDS)
def test_network_exclusion(*args):
    """
    Exclude networks using regular expressions.
    """
    with mock.patch(
        'khulnasoft_checks.openstack.OpenStackCheck.get_stats_for_single_network'
...
@mock.patch(
    'khulnasoft_checks.openstack.OpenStackCheck._make_request_with_auth_fallback', return_value=common.MOCK_NOVA_SERVERS
)
@mock.patch(
    'khulnasoft_checks.openstack.OpenStackCheck.get_nova_endpoint',
    return_value="http://10.0.2.15:8774/v2.1/0850707581fe4d738221a72db0182876",
)
@mock.patch('khulnasoft_checks.openstack.OpenStackCheck.get_auth_token', return_value="test_auth_token")
@mock.patch('khulnasoft_checks.openstack.OpenStackCheck.get_project_name_from_id', return_value="tenant-1")
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            [{}],
            r'khulnasoft_checks.base.errors.ConfigurationError: Detected 2 errors',
            id="empty instance",
        ),
        pytest.param(
            {
            ...
            [{'name': 'test'}],
            r'khulnasoft_checks.base.errors.ConfigurationError: Detected 1 error',
            id="no user",
        ),
        pytest.param(
            {
