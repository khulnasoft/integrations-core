...
from khulnasoft_checks.openstack_controller.legacy.api import AbstractApi, Authenticator, SimpleApi
from khulnasoft_checks.openstack_controller.legacy.exceptions import IncompleteConfig, KeystoneUnreachable
from khulnasoft_checks.openstack_controller.legacy.openstack_controller_legacy import OpenStackControllerLegacyCheck
...
def test_api_error_log_no_password(check, instance, caplog):
    ...
    with caplog.at_level(logging.DEBUG):
        with pytest.raises(KeystoneUnreachable):
            with mock.patch('khulnasoft_checks.base.utils.http.requests.post') as req:
...
@mock.patch(
    'khulnasoft_checks.openstack_controller.legacy.openstack_controller_legacy.OpenStackControllerLegacyCheck.get_servers_detail',
...
@mock.patch(
    'khulnasoft_checks.openstack_controller.legacy.api.ApiFactory.create', return_value=mock.MagicMock(AbstractApi)
)
...
@mock.patch(
    'khulnasoft_checks.openstack_controller.legacy.openstack_controller_legacy.OpenStackControllerLegacyCheck.get_server_diagnostics',
    side_effect=get_server_diagnostics_pre_2_48_response,
)
@mock.patch(
    'khulnasoft_checks.openstack_controller.legacy.openstack_controller_legacy.OpenStackControllerLegacyCheck.get_os_aggregates',
...
def test_get_keystone_url_from_khulnasoft_config():
    check = OpenStackControllerLegacyCheck(
        "test", {'ssl_verify': False, 'paginated_server_limit': 1}, [common.KEYSTONE_INSTANCE]
    )
