...
from khulnasoft_checks.openstack_controller.legacy.api import ApiFactory, Authenticator, Credential, SimpleApi
from khulnasoft_checks.openstack_controller.legacy.exceptions import (
...
def test_get_roles():
    ...
    expected_roles = ['khulnasoft_monitoring', '_member_']
...
def test_from_config(requests_wrapper):
    ...
    with mock.patch(
        'khulnasoft_checks.openstack_controller.legacy.api.Authenticator._post_auth_token', return_value=mock_response
    ):
        with mock.patch(
            'khulnasoft_checks.openstack_controller.legacy.api.Authenticator._get_auth_projects',
...
def test_from_config_with_admin(requests_wrapper):
    ...
    mock_http_response['token']['roles'] = [
        ...
        {u'id': u'f20c215f5a4d47b7a6e510bc65485ced', u'name': u'khulnasoft_monitoring'},
    ]
    mock_response = MockHTTPResponse(response_dict=mock_http_response, headers={'X-Subject-Token': 'fake_token'})
...
def test_get_os_hypervisor_uptime(aggregator, requests_wrapper):
    ...
    with mock.patch(
        'khulnasoft_checks.openstack_controller.legacy.api.SimpleApi._make_request',
...
def test_get_os_aggregates(aggregator, requests_wrapper):
    with mock.patch(
        'khulnasoft_checks.openstack_controller.legacy.api.SimpleApi._make_request', side_effect=get_os_aggregates_response
    ):
        api = SimpleApi(None, None, requests_wrapper)
...
def test__get_paginated_list(requests_wrapper):
    ...
    with mock.patch("khulnasoft_checks.openstack_controller.legacy.api.SimpleApi.connect"):
...
def test__make_request_failure(requests_wrapper):
    ...
    with mock.patch("khulnasoft_checks.openstack_controller.legacy.api.requests.get", return_value=response_mock):
