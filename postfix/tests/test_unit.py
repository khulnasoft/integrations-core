...
from khulnasoft_checks.postfix import PostfixCheck
...
def test__get_postqueue_stats(aggregator):
    ...
    with mock.patch('khulnasoft_checks.postfix.postfix.get_subprocess_output') as s:
...
@mock.patch(
    'khulnasoft_checks.postfix.postfix.get_subprocess_output',
    return_value=('mail_version = {}'.format(MOCK_VERSION), None, None),
)
def test_collect_metadata(aggregator, khulnasoft_agent):
