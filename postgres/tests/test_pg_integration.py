...
from khulnasoft_checks.base.errors import ConfigurationError
from khulnasoft_checks.postgres import PostgreSql
from khulnasoft_checks.postgres.__about__ import __version__
from khulnasoft_checks.postgres.util import PartialFormatter, fmt
...
def test_connections_metrics(aggregator, integration_check, pg_instance):
    ...
    expected_tags += ['db:khulnasoft_test']
...
@requires_static_version
def test_version_metadata(integration_check, pg_instance, khulnasoft_agent):
    check = integration_check(pg_instance)
    ...
    khulnasoft_agent.assert_metadata_count(5)  # for raw and patch
...
def test_state_clears_on_connection_error(integration_check, pg_instance):
    ...
    with mock.patch('khulnasoft_checks.postgres.PostgreSql._collect_stats', side_effect=throw_exception_first_time):
...
def test_correct_hostname(dbm_enabled, reported_hostname, expected_hostname, aggregator, pg_instance):
    ...
    with mock.patch(
        'khulnasoft_checks.postgres.PostgreSql.resolve_db_host', return_value=expected_hostname
    ) as resolve_db_host:
...
def test_database_instance_cloud_metadata_aws(
    ...
    if not expected_error:
        ...
        with mock.patch('khulnasoft_checks.postgres.aws.generate_rds_iam_token') as mocked_generate_rds_iam_token:
...
def test_database_instance_cloud_metadata_azure(
    ...
    if not expected_error:
        ...
        # this is because boto3.generate_rds_iam_token will always return a token (presigned url)
        with mock.patch(
            'khulnasoft_checks.postgres.azure.generate_managed_identity_token'
