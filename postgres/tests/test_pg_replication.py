...
def test_conflicts_lock(aggregator, integration_check, pg_instance, pg_replica_instance2):
    ...
    _wait_for_value(
        ...
        lower_threshold=0,
        query="select confl_lock from pg_stat_database_conflicts where datname='khulnasoft_test';",
    )
...
def test_conflicts_snapshot(aggregator, integration_check, pg_instance, pg_replica_instance2):
    ...
    _wait_for_value(
        pg_replica_instance2,
        lower_threshold=0,
        query="select confl_snapshot from pg_stat_database_conflicts where datname='khulnasoft_test';",
    )
    check.check(pg_replica_instance2)
...
def test_conflicts_bufferpin(aggregator, integration_check, pg_instance, pg_replica_instance2):
    ...
    _wait_for_value(
        pg_replica_instance2,
        lower_threshold=0,
        query="select confl_bufferpin from pg_stat_database_conflicts where datname='khulnasoft_test';",
    )
