...
from khulnasoft_checks.base.utils.db.sql import compute_sql_signature
from khulnasoft_checks.base.utils.db.utils import DBMAsyncJob
from khulnasoft_checks.base.utils.serialization import json
from khulnasoft_checks.base.utils.time import UTC
from khulnasoft_checks.postgres.statement_samples import (
    DBExplainError,
    StatementTruncationState,
)
from khulnasoft_checks.postgres.statements import PG_STAT_STATEMENTS_METRICS_COLUMNS, PG_STAT_STATEMENTS_TIMING_COLUMNS
from khulnasoft_checks.postgres.util import payload_pg_version
from khulnasoft_checks.postgres.version_utils import V12
...
SAMPLE_QUERIES = [
    ...
    ("bob", "bob", "khulnasoft_test", "SELECT city FROM persons WHERE city = %s", "hello"),
    (
        "bob",
        "bob",
        "khulnasoft_test",
...
def test_statement_metrics_version(integration_check, dbm_instance, version, expected_payload_version):
    if version:
        ...
        with mock.patch(
            'khulnasoft_checks.postgres.postgres.PostgreSql.load_version', new_callable=mock.MagicMock
        ) as patched_version:
...
def test_statement_metrics(
    ...
    assert event['ddagentversion'] == khulnasoft_agent.get_version()
    assert event['ddagenthostname'] == khulnasoft_agent.get_hostname()
    ...
    for username, _, dbname, query, _ in SAMPLE_QUERIES:
        ...
        assert fqt_event['ddagentversion'] == khulnasoft_agent.get_version()
...
def test_statement_metrics_cloud_metadata(
    aggregator, integration_check, dbm_instance, input_cloud_metadata, output_cloud_metadata, khulnasoft_agent
):
...
def test_statement_metrics_with_duplicates(aggregator, integration_check, dbm_instance, khulnasoft_agent):
    # don't need samples for this test
    dbm_instance['query_samples'] = {'enabled': False}
    dbm_instance['query_activity'] = {'enabled': False}
    # very low collection interval for test purposes
    ...
    with check.db() as conn:
        with conn.cursor() as cursor:
            with mock.patch.object(khulnasoft_agent, 'obfuscate_sql', passthrough=True) as mock_agent:
...
def test_statement_metadata(
    ...
    check = integration_check(dbm_instance)
    check._connect()
    conn = psycopg2.connect(host=HOST, dbname="khulnasoft_test", user="bob", password="bob")
    cursor = conn.cursor()
...
def test_statement_run_explain_errors(
    ...
    run_one_check(check, dbm_instance)
    _, explain_err_code, err = check.statement_samples._run_and_track_explain("khulnasoft_test", query, query, query)
    run_one_check(check, dbm_instance)
...
def test_load_pg_settings(aggregator, integration_check, dbm_instance, db_user):
    ...
    check = integration_check(dbm_instance)
    check._connect()
    if db_user == 'khulnasoft_no_catalog':
        aggregator.assert_metric(
            "dd.postgres.error",
...
@pytest.mark.parametrize(
    ...
    [("bob", "bob", "khulnasoft_test", "BEGIN TRANSACTION; SELECT city FROM persons WHERE city = %s;", "hello")],
)
    ...
    [
        (
            ...
            [
                ...
                "not loaded in database 'khulnasoft_test'. See https://docs.khulnasoft.com/database_monitoring/"
                'setup_postgres/troubleshooting#pg-stat-statements-not-loaded'
                ' for more details\ncode=pg-stat-statements-not-loaded dbname=khulnasoft_test host=stubbed.hostname',
            ],
        ),
        (
            ...
            [
                ...
                "created in database 'khulnasoft_test'. See https://docs.khulnasoft.com/database_monitoring/"
                'setup_postgres/troubleshooting#pg-stat-statements-not-created'
                ' for more details\ncode=pg-stat-statements-not-created dbname=khulnasoft_test host=stubbed.hostname',
            ],
        ),
        (
            ...
            [
                "Unable to collect statement metrics because of an error running queries in database 'khulnasoft_test'. "
                "See https://docs.khulnasoft.com/database_monitoring/troubleshooting for help: cannot insert into view\n"
                "dbname=khulnasoft_test host=stubbed.hostname"
            ],
        ),
        (
            ...
            [
                "Unable to collect statement metrics because of an error running queries in database 'khulnasoft_test'. "
                'See https://docs.khulnasoft.com/database_monitoring/troubleshooting for help: connection reset\n'
                'dbname=khulnasoft_test host=stubbed.hostname',
            ],
        ),
        (
...
def test_statement_metrics_database_errors(
    ...
    with mock.patch(
        'khulnasoft_checks.postgres.statements.PostgresStatementMetrics._get_pg_stat_statements_columns',
    ...
    [
        (
            ...
            [
                ...
                'this warning. See https://docs.khulnasoft.com/database_monitoring/setup_postgres/troubleshooting#'
                'high-pg-stat-statements-max-configuration for more details\n'
                'code=high-pg-stat-statements-max-configuration dbname=khulnasoft_test host=stubbed.hostname '
