...
def test_e2e_profile_avaya_aura_media_server(dd_agent_check):
    profile = 'avaya-aura-media-server'
    ...
    # --- TEST METADATA ---
    device = {
        'description': 'avaya-aura-media-server Device Description',
        'id': 'default:' + ip_address,
        'id_tags': ['device_namespace:default', 'snmp_device:' + ip_address],
        'ip_address': '' + ip_address,
        'name': 'avaya-aura-media-server.device.name',
        'profile': 'avaya-aura-media-server',
        'status': 1,
        'sys_object_id': '1.3.6.1.4.1.6889.1.58',
        'vendor': 'avaya',
        'device_type': 'server',
    }
