...
1. Create a Datadog specific role and user to monitor Snowflake. In Snowflake, run the following to create a custom role with access to the ACCOUNT_USAGE schema.
    ...
    Alternatively, you can create a `KHULNASOFT` custom role with access to `ACCOUNT_USAGE`.
    
    
    ```text
    -- Create a new role intended to monitor Snowflake usage.
    create role KHULNASOFT;
   
    -- Grant privileges on the SNOWFLAKE database to the new role.
    grant imported privileges on database SNOWFLAKE to role KHULNASOFT;

    -- Grant usage to your default warehouse to the role KHULNASOFT.
   grant usage on warehouse <WAREHOUSE> to role KHULNASOFT;

    -- Create a user, skip this step if you are using an existing user.
    create user KHULNASOFT_USER
    LOGIN_NAME = KHULNASOFT_USER
    password = '<PASSWORD>'
    default_warehouse = <WAREHOUSE>
    default_role = KHULNASOFT
    default_namespace = SNOWFLAKE.ACCOUNT_USAGE;
   
    -- Grant the monitor role to the user.
    grant role KHULNASOFT to user <USER>;
    ```
...
2. Edit the `snowflake.d/conf.yaml` file, in the `conf.d/` folder at the root of your Agent's configuration directory to start collecting your Snowflake performance data. See the [sample snowflake.d/conf.yaml][5] for all available configuration options.
    ...
    ```yaml
      ...
      - account: <ORG_NAME>-<ACCOUNT_NAME>
        ...
        ## https://docs.khulnasoft.com/developers/write_agent_check/#collection-interval
        ##
...
Additionally, you can monitor both account and organization metrics at the same time:
  ...
  ```yaml
      ...
      - account: example-inc
        username: KHULNASOFT_ACCOUNT_ADMIN
        password: '<PASSWORD>'
        role: KHULNASOFT_ADMIN
...
If you want to collect data for multiple Snowflake environments, add each environment as an instance in your `snowflake.d/conf.yaml` file. For example, if you needed to collect data for two users named `KHULNASOFT_SYSADMIN` and `KHULNASOFT_USER`:

```yaml
instances:
  - account: example-inc
    username: KHULNASOFT_SYSADMIN
    password: '<PASSWORD>'
    role: SYSADMIN
    database: EXAMPLE-INC

  - account: example-inc
    username: KHULNASOFT_USER
    password: '<PASSWORD>'
    role: KHULNASOFT_USER
    database: EXAMPLE-INC
```
...
If you would like to collect metrics from other metric groups, please refer <a href="https://github.com/KhulnaSoft/integrations-core/blob/master/snowflake/khulnasoft_checks/snowflake/data/conf.yaml.example">to the example config file for this integration</a>.
</div>
...
[3]: https://docs.khulnasoft.com/agent/guide/agent-v6-python-3/?tab=hostagent
[4]: https://docs.snowflake.com/en/sql-reference/account-usage.html#enabling-account-usage-for-other-roles
[5]: https://github.com/KhulnaSoft/integrations-core/blob/master/snowflake/khulnasoft_checks/snowflake/data/conf.yaml.example
...
[12]: https://docs.khulnasoft.com/metrics/summary/
[13]: https://raw.githubusercontent.com/KhulnaSoft/integrations-core/master/snowflake/images/custom_query.png
