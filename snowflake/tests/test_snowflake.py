...
def test_token_path(dd_run_check, aggregator):
    ...
    with mock.patch(
        'khulnasoft_checks.snowflake.check.open',
        side_effect=[mock.mock_open(read_data=tok).return_value for tok in tokens],
    ), mock.patch('khulnasoft_checks.snowflake.check.sf') as sf:
        dd_run_check(check)
...
def test_query_metrics(dd_run_check, aggregator, instance):
    ...
    with mock.patch('khulnasoft_checks.snowflake.SnowflakeCheck.execute_query_raw', return_value=expected_query_metrics):
        check = SnowflakeCheck(CHECK_NAME, {}, [instance])
...
def test_version_metadata(dd_run_check, instance, khulnasoft_agent):
    ...
    with mock.patch('khulnasoft_checks.snowflake.SnowflakeCheck.execute_query_raw', return_value=iter(expected_version)):
        check = SnowflakeCheck(CHECK_NAME, {}, [instance])
