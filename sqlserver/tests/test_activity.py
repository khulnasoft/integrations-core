...
from khulnasoft_checks.base.utils.db.utils import DBMAsyncJob, default_json_event_encoding
from khulnasoft_checks.sqlserver import SQLServer
from khulnasoft_checks.sqlserver.activity import DM_EXEC_REQUESTS_COLS, _hash_to_hex
...
def test_collect_load_activity(
    ...
    assert blocked_row['database_name'] == "khulnasoft_test", "incorrect database_name"
...
def test_activity_nested_blocking_transactions(
    ...
    def run_queries(conn, queries):
        cur = conn.cursor()
        cur.execute("USE {}".format("khulnasoft_test"))
    ...
    assert root_blocker["database_name"] == "khulnasoft_test"
    ...
    assert tx2["database_name"] == "khulnasoft_test"
    ...
    assert tx3["database_name"] == "khulnasoft_test"
...
def test_activity_metadata(
    aggregator, instance_docker, dd_run_check, dbm_instance, khulnasoft_agent, metadata, expected_metadata_payload
):
    check = SQLServer(CHECK_NAME, {}, [dbm_instance])
...
def test_truncate_on_max_size_bytes(dbm_instance, khulnasoft_agent, rows, expected_len, expected_users):
    check = SQLServer(CHECK_NAME, {}, [dbm_instance])
    with mock.patch.object(khulnasoft_agent, 'obfuscate_sql', passthrough=True) as mock_agent:
...
@pytest.mark.unit
def test_activity_stored_procedure_failed_to_obfuscate(dbm_instance, khulnasoft_agent):
    check = SQLServer(CHECK_NAME, {}, [dbm_instance])
    with mock.patch.object(khulnasoft_agent, 'obfuscate_sql', passthrough=True) as mock_agent:
...
def test_activity_stored_procedure_characters_limit(
    ...
    def run_test_query(c, q):
        cur = c.cursor()
        cur.execute("USE khulnasoft_test")
        cur.execute(q)
