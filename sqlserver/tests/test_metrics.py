...
def test_check_index_usage_metrics(
    ...
    instance_docker_metrics['database'] = 'khulnasoft_test'
    instance_docker_metrics['include_index_usage_metrics'] = True

    # Cause an index seek
    bob_conn.execute_with_retries(
        query="SELECT * FROM khulnasoft_test.dbo.ϑings WHERE name = 'foo'",
    ...
    # Cause an index scan
    bob_conn.execute_with_retries(
        query="SELECT * FROM khulnasoft_test.dbo.ϑings WHERE name LIKE '%foo%'",
        database=instance_docker_metrics['database'],
        retries=1,
        return_result=False,
    )
    ...
    bob_conn.execute_with_retries(
        query="SELECT id FROM khulnasoft_test.dbo.ϑings WHERE name = 'foo'",
        database=instance_docker_metrics['database'],
        retries=1,
        return_result=False,
    )
    ...
    bob_conn.execute_with_retries(
        query="UPDATE khulnasoft_test.dbo.ϑings SET id = 1 WHERE name = 'foo'",
        database=instance_docker_metrics['database'],
        retries=1,
        return_result=False,
    )
