...
from khulnasoft_checks.sqlserver.statements import SQL_SERVER_QUERY_METRICS_COLUMNS, obfuscate_xml_plan
...
test_statement_metrics_and_plans_parameterized = (
    ...
    [
        ...
        [
            "master",
            "SELECT * FROM khulnasoft_test.dbo.ϑings where id = ?",
            [r"SELECT \* FROM khulnasoft_test.dbo.ϑings where id = @P1"],
            (
                (1,),
                (2,),
                (3,),
...
def test_statement_metrics_and_plans(
    ...
    assert payload['ddagenthostname'] == khulnasoft_agent.get_hostname()
...
def test_statement_metadata(
    aggregator, dd_run_check, dbm_instance, bob_conn, khulnasoft_agent, metadata, expected_metadata_payload
):
    check = SQLServer(CHECK_NAME, {}, [dbm_instance])
...
def test_statement_cloud_metadata(
    aggregator, dd_run_check, dbm_instance, bob_conn, khulnasoft_agent, input_cloud_metadata, output_cloud_metadata
):
    ...
    def _run_query():
        bob_conn.execute_with_retries(query, (), database="khulnasoft_test")
...
def test_statement_reported_hostname(
    aggregator, dd_run_check, dbm_instance, bob_conn, khulnasoft_agent, reported_hostname, expected_hostname
):
...
@pytest.mark.integration
@pytest.mark.usefixtures('dd_environment')
def test_statement_basic_metrics_query(khulnasoft_conn_docker, dbm_instance):
    now = time.time()
    ...
    with khulnasoft_conn_docker.cursor() as cursor:
...
def test_obfuscate_xml_plan(test_file, obfuscated_file, khulnasoft_agent):
...
def test_statement_stored_procedure_characters_limit(
    aggregator, khulnasoft_agent, dd_run_check, dbm_instance, bob_conn, stored_procedure_characters_limit
):
