...
from khulnasoft_checks.dev.utils import running_on_windows_ci
...
class HighCardinalityQueries:
    ...
    def is_ready(self):
        ...
        cursor.execute(
            'SELECT COUNT(*) FROM khulnasoft_test.sys.database_principals WHERE name LIKE \'high_cardinality_user_%\''
        )
        user_count = cursor.fetchone()[0]
        cursor.execute('SELECT COUNT(*) FROM khulnasoft_test.sys.schemas WHERE name LIKE \'high_cardinality_schema%\'')
        schema_count = cursor.fetchone()[0]
        cursor.execute('SELECT COUNT(*) FROM khulnasoft_test.sys.tables')
        table_count = cursor.fetchone()[0]
        cursor.execute('SELECT COUNT(*) FROM khulnasoft_test.dbo.high_cardinality')
    ...
    def start_background(self, config=None):
        """
        Run a set of queries against the table `khulnasoft_test.dbo.high_cardinality` in the background
    ...
    def create_high_cardinality_query(self):
        ...
        shuffle(columns)
        return 'SELECT {col} FROM khulnasoft_test.dbo.high_cardinality WHERE id = {id}'.format(
    ...
    def create_slow_query(self):
        ...
        columns = copy(self.columns)
        shuffle(columns)
        return 'SELECT TOP 10 {col} FROM khulnasoft_test.dbo.high_cardinality WHERE col2_txt LIKE \'%{pattern}%\''.format(
    ...
    def create_complex_query(self):
        ...
        SELECT
            {col}
        FROM
            khulnasoft_test.dbo.high_cardinality AS hc1
            JOIN (
                SELECT
                    id,
                    COUNT(*) col12_float
                FROM
                    khulnasoft_test.dbo.high_cardinality AS hc2
                WHERE
                    hc2.col1_txt LIKE '%-%'
                    AND hc2.col14_int > (
                        SELECT
                            AVG(hc3.col15_int)
                        FROM
                            khulnasoft_test.dbo.high_cardinality AS hc3)
                    GROUP BY
                        hc2.id) AS hc4 ON hc4.id = hc1.id
            JOIN khulnasoft_test.dbo.high_cardinality AS hc5 ON hc5.id = hc1.id
        WHERE
