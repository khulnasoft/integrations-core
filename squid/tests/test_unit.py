...
from khulnasoft_checks.squid import SquidCheck
...
def test_get_counters(check):
    ...
    See https://github.com/KhulnaSoft/integrations-core/pull/1643
    """
    with mock.patch('khulnasoft_checks.squid.squid.requests.get') as g:
        with mock.patch('khulnasoft_checks.squid.SquidCheck.submit_version'):
...
@pytest.mark.parametrize(
    'auth_config',
    [
        {"cachemgr_username": "khulnasoft_user", "cachemgr_password": "khulnasoft_pass"},
        {"username": "khulnasoft_user", "password": "khulnasoft_pass"},
    ],
)
def test_legacy_username_password(instance, auth_config):
    ...
    with mock.patch('khulnasoft_checks.base.utils.http.requests.get') as g:
        with mock.patch('khulnasoft_checks.squid.SquidCheck.submit_version'):
            check.get_counters('host', 'port', [])

            g.assert_called_with(
                'http://host:port/squid-internal-mgr/counters',
                auth=('khulnasoft_user', 'khulnasoft_pass'),
                cert=mock.ANY,
                headers=mock.ANY,
                proxies=mock.ANY,
                timeout=mock.ANY,
