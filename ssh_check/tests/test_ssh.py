...
from khulnasoft_checks.ssh_check import CheckSSH
...
def test_collect_metadata(version, metadata, khulnasoft_agent):
    client = MagicMock()
    ...
    khulnasoft_agent.assert_metadata('test:123', metadata)


def test_collect_bad_metadata(khulnasoft_agent):
    client = MagicMock()
    ...
    ssh.check_id = 'test:123'
    ssh._collect_metadata(client)
    khulnasoft_agent.assert_metadata_count(1)
    khulnasoft_agent.assert_metadata('test:123', {'flavor': 'unknown'})
