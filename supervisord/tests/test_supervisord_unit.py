...
from khulnasoft_checks.supervisord.supervisord import FORMAT_TIME  # pylint: disable=import-error,no-name-in-module
...
def test_version_metadata_pattern(check, khulnasoft_agent, raw_version, expected_metadata, expected_count):
    check.check_id = 'test:123'
    supe = mock.MagicMock()
    supe.getSupervisorVersion.return_value = raw_version
    check._collect_metadata(supe)

    khulnasoft_agent.assert_metadata('test:123', expected_metadata)
    khulnasoft_agent.assert_metadata_count(expected_count)
