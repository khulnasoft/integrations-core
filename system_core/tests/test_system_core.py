...
from khulnasoft_checks.system_core import SystemCore
...
class TestSystemCore:
    def test_system_core(self, aggregator):
        ...
        with mock.patch('khulnasoft_checks.system_core.system_core.psutil') as psutil_mock:
    ...
    def test_system_core_not_unix(self, aggregator, dd_run_check):
        ...
        """
        c = SystemCore('system_core', {}, {}, [{}])
        with mock.patch('khulnasoft_checks.system_core.system_core.psutil') as psutil_mock, mock.patch.object(
            Platform, 'is_unix'
        ) as is_unix_mock:
            psutil_mock.cpu_times.side_effect = fake_cpu_times
            psutil_mock.cpu_count.return_value = common.MOCK_PSUTIL_CPU_COUNT
