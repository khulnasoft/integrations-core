...
from khulnasoft_checks.tcp_check import TCPCheck

CHECK_NAME = "tcp_check"

INSTANCE = {'host': 'khulnasoft.com', 'port': 80, 'timeout': 1.5, 'name': 'UpService', 'tags': ["foo:bar"]}

INSTANCE_MULTIPLE = {'multiple_ips': True}
INSTANCE_MULTIPLE.update(INSTANCE)

INSTANCE_IPV6 = {
    'host': 'ip-ranges.khulnasoft.com',
    'port': 80,
    'timeout': 5,
    'name': 'UpService',
    'tags': ["foo:bar"],
...
def _test_check(aggregator, addrs):
    common_tags = ['foo:bar', 'target_host:khulnasoft.com', 'port:80', 'instance:UpService']
    for addr in addrs:
