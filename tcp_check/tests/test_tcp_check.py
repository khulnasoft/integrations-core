...
from khulnasoft_checks.tcp_check.tcp_check import AddrTuple, TCPCheck
...
def test_up(aggregator, check):
    ...
    expected_tags = ["instance:UpService", "target_host:khulnasoft.com", "port:80", "foo:bar"]
...
def test_response_time(aggregator):
    ...
    # service check
    expected_tags = ['foo:bar', 'target_host:khulnasoft.com', 'port:80', 'instance:instance:response_time']
    expected_tags.append("address:{}".format(check._addrs[0].address))
    aggregator.assert_service_check('tcp.can_connect', status=check.OK, tags=expected_tags)
    aggregator.assert_metric('network.tcp.can_connect', value=1, tags=expected_tags)

    # response time metric
    expected_tags = ['url:khulnasoft.com:80', 'instance:instance:response_time', 'foo:bar']
    expected_tags.append("address:{}".format(check._addrs[0].address))
...
def test_multiple(aggregator):
    ...
    with mock.patch(
        ...
        expected_tags = ['foo:bar', 'target_host:khulnasoft.com', 'port:80', 'instance:multiple']
...
def test_ipv6(aggregator, check):
    ...
    for addr in check.addrs:
        expected_tags = ["instance:UpService", "target_host:ip-ranges.khulnasoft.com", "port:80", "foo:bar"]
