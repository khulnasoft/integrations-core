...
def test_check_with_events(
    dd_run_check, khulnasoft_agent, aggregator, check, mocked_management_instance, mocker, new_response, expected_events
):
...
def test_check_disable_events(dd_run_check, khulnasoft_agent, aggregator, check, mocked_management_instance, mocker):
    # We generate a cache to save the models, so clear it to make sure we won't get the data from a previous test.
    khulnasoft_agent.reset()
