...
from khulnasoft_checks.vsphere.constants import DEFAULT_MAX_QUERY_METRICS
...
def mock_type():
    ...
    """
    paths = [
        'khulnasoft_checks.vsphere.cache.type',
        'khulnasoft_checks.vsphere.utils.type',
        'khulnasoft_checks.vsphere.vsphere.type',
        'khulnasoft_checks.vsphere.api_rest.type',
    ]
...
@pytest.fixture
def mock_threadpool():
    with patch('khulnasoft_checks.vsphere.vsphere.ThreadPoolExecutor') as pool, patch(
        'khulnasoft_checks.vsphere.vsphere.as_completed', side_effect=lambda x: x
    ):
...
@pytest.fixture
def mock_api():
    with patch('khulnasoft_checks.vsphere.vsphere.VSphereAPI', MockedAPI):
        yield
...
@pytest.fixture
def get_timestamp():
    with patch('khulnasoft_checks.vsphere.vsphere.get_timestamp') as mock_time:
        yield mock_time
