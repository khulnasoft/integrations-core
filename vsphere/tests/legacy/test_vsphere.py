...
from khulnasoft_checks.vsphere.legacy.cache_config import CacheConfig
from khulnasoft_checks.vsphere.legacy.common import SOURCE_TYPE
from khulnasoft_checks.vsphere.legacy.errors import BadConfigError, ConnectionError
from khulnasoft_checks.vsphere.legacy.vsphere_legacy import (
...
def test_excluded_host_tags(vsphere, instance, aggregator):
    ...
    with mock.patch(
        "khulnasoft_checks.vsphere.legacy.vsphere_legacy.VSphereLegacyCheck._collect_mors_and_attributes",
...
def test__collect_mors_and_attributes(vsphere, instance):
    ...
    """
    server_instance = vsphere._get_server_instance(instance)
    with mock.patch("khulnasoft_checks.vsphere.legacy.vsphere_legacy.vmodl"):
...
def test_use_guest_hostname(vsphere, instance):
    # Default value
    with mock.patch(
        "khulnasoft_checks.vsphere.legacy.vsphere_legacy.VSphereLegacyCheck._get_all_objs"
    ) as mock_get_all_objs, mock.patch("khulnasoft_checks.vsphere.legacy.vsphere_legacy.vmodl"):
        vsphere._cache_morlist_raw(instance)
        # Default value
...
def test_service_check_ko(aggregator, instance):
    ...
    with mock.patch('khulnasoft_checks.vsphere.legacy.vsphere_legacy.connect.SmartConnect') as SmartConnect:
        # SmartConnect fails
