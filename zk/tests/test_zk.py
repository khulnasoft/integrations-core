...
from khulnasoft_checks.zk import ZookeeperCheck
...
def test_metadata(khulnasoft_agent, get_test_instance):
    check = ZookeeperCheck(conftest.CHECK_NAME, {}, [get_test_instance])
...
def test_metadata_regex(khulnasoft_agent, get_test_instance):
    check = ZookeeperCheck(conftest.CHECK_NAME, {}, [get_test_instance])
    check.check_id = 'test:123'
    check.check(get_test_instance)
    if PY2:
